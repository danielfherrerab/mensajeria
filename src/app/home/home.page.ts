...
export class HomePage implements OnInit {
  ...
  constructor (
    ...
    saveEvent(): void {
        ...
        if (this.isEdit) {
            ...
            const newEvent = {
                ...
                title: this.tempEvent.title,
                description: this.tempEvent.description,
                allDay: this.tempEvent.allDay,
                free: this.tempEvent.free,
                color: this.tempEvent.color
              };
        } else {
    ...
    ngOnInit() {
        this.orderListService.getOrders().subscribe((orders: OrderData[]) => {
            this.myEvents = orders.map((order: OrderData) => {
                let dateObject = new Date(order.fechaRequerida.seconds * 1000);
                console.log(dateObject, order.id, order.descripcion);
                return {
                    id: order.id,
                    start: dateObject,
                    end: dateObject,
